...
#-----------------------------------------------------------------------------------------
#-----------------------------------------------------------------------------------------
#SBO1- Frontiers Analysis: ships dominate SPL levels, even when biological present!
#-----------------------------------------------------------------------------------------
#-----------------------------------------------------------------------------------------
#PLOT: tile plot for each component order by SPL to show different combinations for different sound levels
DayTrunData  = mutate_if(DayTrunData, is.factor, ~ as.numeric(as.character(.x)))
...
#summarize bioloical
DayTrunData$BioTotal = rowSums(DayTrunData[,33:37]) + DayTrunData[,40]
#combine just 125, AIS, WSPD, BIO
DayTrunDataT = ( DayTrunData[c(18,11,12,15,43,26)] )
...
#plot as tile for each componenet
...
#PLOT: SPL vs AIS vessels (hourly)- color by windspeed to show wind can also lead to higher levels
# these should explain outliers!!
ggplot(HrTrunData, aes(OL_125, totalTime, color = as.factor(round(avgWSPD)) ) ) +
  geom_point()+
  geom_smooth()
library(plyr)
ggplot(DayTrunData, aes(mOB_125, VessD_Time_PerDay )) +
  geom_point()+
  geom_smooth(method="lm",formula= y~x,se = FALSE,color = "black",alpha = 0.5)+
  xlab("median SPL 125 Hz octave band")+
  ylab("Proportion of day dominated by vessel noise")+
  geom_point(data = DayTrunData, mapping = aes(mOB_125, VessD_Time_PerDay,color = as.factor(round_any(WSPD_mean,5)) ) ) +
  labs(col="Wind speed")
#PLOT: occurence plot for bio, physical, anthro
trunDataMelt = reshape :: melt(DayTrunData, id.vars = "Day", 
                               measure.vars = c("bluewhaleP","finwhaleP","humpbackwhaleP",
                                                "northatlanticrightwhaleP","seiwhaleP", 
                                                "dolphinsPscale","atlanticcodPscale" ))
trunDataMelt$value = as.numeric(as.character(trunDataMelt$value) )
#biological
p1t = ggplot(trunDataMelt, aes(Day, variable, fill= (value))) + 
  geom_tile() +
  scale_fill_gradient(low="white", high="blue") +
  scale_y_discrete(labels = c("BW","FW","HW","RW","SW","DO","CD")) +
...
#wind, vessels, as bars...
abioDaym = reshape :: melt(DayTrunData, id.vars = "Day", 
                           measure.vars = c("WSPD_mean","LOA_ALL_UV"))
abioDaym$value = as.numeric(as.character (abioDaym$value) )
abioDaymWind = abioDaym[ abioDaym$variable == "WSPD_mean", ]
pWl =   ggplot(abioDaymWind, aes(x = Day, y = value ) ) + 
  geom_bar(stat="identity") +
  xlab("") +
  #scale_x_discrete(labels = "")+
  ylab("Wind speed [kts]")
abioDaymWind = abioDaym[ abioDaym$variable == "LOA_ALL_UV", ]
pAl =   ggplot(abioDaymWind, aes(x = Day, y = value ) ) + 
  geom_bar(stat="identity") +
  xlab("") +
  ylab("AIS ships (total unique)")
#combine graphics
grid.arrange(p1t,pWl,pAl,nrow=3,ncol=1)

#as tiles... no that helpflut
abioDaymWind = abioDaym[ abioDaym$variable == "WSPD_mean", ]
pWl = ggplot(abioDaymWind, aes(Day, variable, fill= (value)) ) + 
  geom_tile() +
  scale_fill_gradient(low="white", high="blue") +
  scale_y_discrete(labels = "WS")+
  labs(fill = "Wind Speed [mps]") +
  xlab("") +
  ylab("")+
  theme(legend.position="top", axis.text.x = element_blank())
abioDaymVess = abioDaym[ abioDaym$variable == "LOA_ALL_OPHRS", ]
pAIS= ggplot(abioDaymVess, aes(Day, variable, fill= (value)) ) + 
  geom_tile() +
  scale_fill_gradient(low="white", high="blue") +
  scale_y_discrete(labels = "SH")+
  labs(fill = "Operation Hours") +
  xlab("") +
  ylab("")+
  theme(legend.position="top", axis.text.x = element_blank())
abioDaymVess = abioDaym[ abioDaym$variable == "VessD_Time_PerDay", ]
pVES= ggplot(abioDaymVess, aes(Day, variable, fill= (value)*100) ) + 
  geom_tile() +
  scale_fill_gradient(low="white", high="blue") +
  scale_y_discrete(labels = "VD")+
  labs(fill = "Percent of day vessel noise") +
  xlab("") +
  ylab("")+
  theme(legend.position="top")
#combine graphics
grid.arrange(p1t,pWl,pAIS, pVES,nrow=4,ncol=1)

#DATA EXPLORE (Not used): what are the different combinations of conditions to get the same daily sound level (ugh hourly is so much better!!)
#SB is vessel dominated-- expect higher sound levels with more ships
ggplot(DayTrunData, aes(x=mOB_125, y=LOA_ALL_UV ) )+
  geom_point() 
#not the case... lowest levels (an mid levels)-- medium level of ships (7), but highest levels-- most ships
#unless you account for wind and biological sounds present
# show: linear relationship vs predicted with wind and biologicals?
ggplot(DayTrunData, aes(x=mOB_125, y=LOA_ALL_UV, color = WSPD_mean,shape =as.factor(BioTotal) ) )+
  geom_point()
tst = DayTrunData[DayTrunData$mOB_125 > 100 & DayTrunData$mOB_125 < 105,]
tst = ( tst[c(18,11,43,26)] )

ggplot(HrTrunData, aes(x=Day, y=OL_125, group= Day, fill =as.numeric(as.character(LOA_ALL_UV ))) )+
  geom_boxplot( )

trunDataD = daySum1[( as.Date(daySum1$Day) >= as.Date(tmpAP$DateStart) & as.Date(daySum1$Day) <= as.Date(tmpAP$DateEnd) ),]
ggplot(trunDataD, aes(x=as.numeric( as.character(LOA_L_UV)), y=as.numeric( as.character(mOB_125)) ) )+
  geom_point( )       
