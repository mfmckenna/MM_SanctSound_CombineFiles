...
#add deployment to the spreadhsheet?-- find breaks in data (SEE _saveOut.R version for improved)
...
#AIS new results
nAIS = rbind( c("2019","143","3404"), c("2020","110","2459"))
colnames(nAIS) = c("year","UV","TT")
nAIS = as.data.frame(nAIS)
nAIS$TT = as.numeric( as.character( nAIS$TT ) )
nAIS$UV = as.numeric( as.character( nAIS$UV ) )
nAIS$year =( as.character( nAIS$year) )
nAIS
pnTT = ggplot(nAIS,aes(x=year,y=TT))+
  geom_bar(stat="identity",fill=c("#E69F00", "#56B4E9"),color="black")+
  xlab("") +  ylab ("") + ggtitle("Total transit distance of \n vessels close to recorder")+
  theme(axis.text = element_text(size=14),
        panel.background = element_rect(fill = "transparent"),
        axis.title=element_text(size=5,face="bold"),
        plot.background = element_rect(fill = "transparent", color = NA),
...
grid.arrange(pSPL, pDet, pnTT, ncol = 3, nrow = 1)

pnUV = ggplot(nAIS,aes(x=year,y=UV))+
  geom_bar(stat="identity",fill=c("#E69F00", "#56B4E9"),color="black")+
  xlab("") +  ylab ("") + ggtitle("Unique vessels tracking \n close to recorder")+
  theme(axis.text = element_text(size=14),
        panel.background = element_rect(fill = "transparent"),
        axis.title=element_text(size=5,face="bold"),
        plot.background = element_rect(fill = "transparent", color = NA),
        plot.margin = unit(c(1,2,2,2), "lines")  ) 
grid.arrange(pSPL, pDet, pnUV, ncol = 3, nrow = 1)
