...
#-----------------------------------------------------------------------------------------
# reads in output from: 1_process_VesselDetectionsAIS.R to incoporate detections....
...
#-----------------------------------------------------------------------------------------
dirSite   = "E:\\RESEARCH\\SanctSound\\data\\MB"
sanctuary = "MB"
sites     = c("01","02") #list sites, MB03 is weird, so leaving out for now!
...
# only plots data for March 2019
# assuming if SPL data exists then detectors were run-- will need to check effort for this!
#-----------------------------------------------------------------------------------------
# LOAD Libraries
#-----------------------------------------------------------------------------------------
library(lubridate)
...
wind_filepart = "MetDat" #"^SST"
tide_filepart = "wl" #Hr2"
...
cat("current data range: ", as.character( min(dataInHr$Day)), " to ", as.character( max(dataInHr$Day)) )
...
multmerge = function(path){
  filenames = list.files(path=path, pattern = wind_filepart, full.names=TRUE, recursive = TRUE)
  rbindlist(lapply(filenames, fread))
}
# list.files(path=dirSite, pattern = wind_filepart, full.names=TRUE, recursive = TRUE)
WIND <- multmerge(dirSite)

#remove 1st row with units
nrow(WIND)
WIND = WIND[2:nrow(WIND),]
as.data.frame( colnames(WIND) )
#format date
WIND$Day = as.Date( paste( WIND$`#YY`, "/", WIND$MM, "/",WIND$DD, " ",WIND$hh, ":",WIND$mm,":00", sep = "") ) 
...
WIND$WSPD = as.numeric( as.character(WIND$WSPD) )
idxNA = which( WIND$WSPD == 99 ) #recode missing data
WIND$WSPD[idxNA] = NA

#data check plot
pWIND = ggplot(WIND, aes(Day, WSPD) ) +
  geom_point() +
  theme_minimal()
#pWIND  #2018 data looks strange-- clips at 6m/s

#summarize/ merge by day
WINDday = aggregate(as.numeric(as.character( WIND$WSPD)),    by=list(WIND$Day), mean, na.rm=T)     #mean windspeed for day
...
WINDhr2 = aggregate(as.numeric(as.character( WINDhr$WSP)),    by=list(WINDhr$DateF), mean, na.rm=T)     #mean windspeed for day
...
#-----------------------------------------------------------------------------------------
multmerge = function(path){
  filenames = list.files(path=path, pattern = tide_filepart, full.names=TRUE, recursive = TRUE)
  rbindlist(lapply(filenames, fread))
}
# list.files(path=dirSite, pattern = tide_filepart, full.names=TRUE, recursive = TRUE)
...
#-----------------------------------------------------------------------------------------
#BLUEWHALE detections: start time only, presence
#-----------------------------------------------------------------------------------------
bFiles = list.files(path=dirSite, pattern = detectionsB[1], full.names=TRUE, recursive = TRUE)
idx = which(!grepl("MB03",bFiles))
bFiles = bFiles[idx]
idx = which(!grepl("manual",bFiles))
bFiles = bFiles[idx]
bluewhale = NULL
for (dd in 1:length(bFiles) ){
  ...
  if (nrow(tmp)>0){
    ...
    #cat(colnames(tmp),"\n")
    if (ncol(tmp) == 5 ){
      bluewhale = rbind(bluewhale, tmp[,2:5] )
      cat(tmp$Site[1],tmp$Dep[1],as.character(tmp$DateTime [1]),"\n")
    } else {cat(paste0("No Detections: ",basename(bFiles[dd]),"\n"))} #skips files without detections
    
  } else   {cat(paste0("No Detections: ",basename(bFiles[dd]),"\n")) } #skips files without detections
  
}
bluewhale$DateDet = as.POSIXct( substr( gsub("T", " ", bluewhale$DateTime),start = 1, stop = 19 ), tz = "GMT" )
bluewhale$DateF   = as.POSIXct( paste0( substr( gsub("T", " ", bluewhale$DateTime),start = 1, stop = 14 ), "00:00"), tz = "GMT" )
bluewhale$Day     = as.Date(bluewhale$DateF)
bluewhale$bluewhaleDet = 1

#SUMMARIZE: by hour/day
bluewhaleHr = aggregate(bluewhaleDet ~ DateF + Dep+ Site, data = bluewhale, FUN = length)
bluewhaleDy = aggregate(bluewhaleDet ~ Day   + Dep+ Site, data = bluewhale, FUN = length)

#MERGE: with data
#dataInDay4 = merge( dataInDay3, bluewhaleDy, by=c("Day","Site") , all.x=TRUE)
dataInDay4 = merge( dataInDay3, bluewhaleDy[,c("Day","Site","bluewhaleDet")], by=c("Day","Site") , all.x=TRUE)
dataInHr4  = merge( dataInHr3,  bluewhaleHr[,c("DateF","Site","bluewhaleDet")], by=c("DateF","Site"), all.x=TRUE)

#RECODE: times with no detections are NA and need to be zeros if data exists
#assuming if data exists then detectors were run....)
...
#-----------------------------------------------------------------------------------------
#Finwhale detections- houly presence with effort :)!
#-----------------------------------------------------------------------------------------
bFiles = list.files(path=dirSite, pattern = detectionsB[2], full.names=TRUE, recursive = TRUE)
idx = which(!grepl("MB03",bFiles))
bFiles = bFiles[idx]
idx = which(!grepl("manual",bFiles))
bFiles = bFiles[idx]
...
for (dd in 1:length(bFiles) ){
  ...
  if (nrow(tmp)>0){
    ...
    if (ncol(tmp) == 6 ){
      finwhale = rbind(finwhale, tmp[,2:6] )
      cat(tmp$Site[1],tmp$Dep[1],as.character(tmp$DateTime [1]),"\n")
    } else {cat(paste0("No Detections: ",basename(bFiles[dd]),"\n"))} #skips files without detections
...
finwhale$DateF = as.POSIXct( substr( gsub("T", " ", finwhale$DateTime),start = 1, stop = 19 ), tz = "GMT" )
finwhale$Day     = as.Date(finwhale$DateF)
finwhale$finwhaleDet = finwhale$presence
finwhale$finwhaleDet[finwhale$Effort == 0] = NA # if no effort change presence to NA

#MERGE: with data, just detection column
dataInHr5  = merge( dataInHr4,  finwhale[,c("DateF","Site","finwhaleDet")], by=c("DateF","Site"), all.x=TRUE)

#summarize by day
finwhaleDy = aggregate(finwhaleDet ~ Day + Site, data = finwhale, FUN = sum)
max(finwhaleDy$finwhaleDet)
#check to see if really only 1 hr with calls per day...suspicious!!
finwhaleP  = finwhale[!is.na(finwhale$finwhaleDet),]
finwhaleP  = finwhaleP[finwhaleP$finwhaleDet == 1,]
dataInDay5 = merge( dataInDay4, finwhaleDy[,c("Day","Site","finwhaleDet")], by=c("Day","Site") , all.x=TRUE)
